...
workbook = xlsxwriter.Workbook('C:/Users/Bogdan/OneDrive/Desktop/PEGS.xlsx')
worksheet = workbook.add_worksheet()

class generateXL:

    #received_PEG = PEG_Item()
    #current_User = User_Item()
    ...
    #print(dict)
    print(dict["result"]["prof_industry_exp"]["rating"])


    def complete_PEG(dict, id):

        average=0
        dict = get_peg(id)

        worksheet.write(0, 1, dict["Fiscal year"])
        print(dict["Fiscal year"])
       # worksheet.write(1, 1, current_User.name)
        worksheet.write(2, 1, dict["User id"])
        print(dict["User id"])
        #worksheet.write(3, 1, current_User.career_level)
        #worksheet.write(4, 1, current_User.organizational_assgmnt)
